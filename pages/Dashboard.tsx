...
const Dashboard = () => {
  ...
  const handleRefresh = async () => {
    ...
    setIsLoading(true);
    try {
      const skills = await generateSkillAnalysis(domain, `Profile: ${user.name}. Skills: ${user.skills.join(', ')}`);
      ...
      if (updatedRoadmaps.length > 0) {
        updatedRoadmaps[0] = { ...updatedRoadmaps[0], skillAnalysis: skills };
      }
      authService.updateUser({ ...user, roadmaps: updatedRoadmaps });
    } catch (error: any) {
      console.error('Skill refresh error:', error);
      alert(`Error updating skills: ${error.message || 'Unknown error occurred'}`);
    } finally {
      setIsLoading(false);
    }
  };
